function [x_decod,prob_b,nub,varb]=DKSEPalg_OptComp(A,complexFlag,sigma,r,h,pui_ldpc,nturbo,nu_E,var_E,L_EP)
...
% Created 29/06/2018
%
% Description: This function runs the D-KSEP [Santos19] for turbo equalization. 
...
AlphabetMat=(diag(A)*ones(M,input_length)).'; % Matrix NxL
...
if ~isempty(nu_E)
    ...
    % from the decoder   
    GAMMAupdated=(1./var_pi)-(1./var_E);
    gammaupdated=(nu_pi./var_pi)-(nu_E./var_E);
    
    % Control of negative variances
    indexNeg=find(GAMMAupdated>0);
    meanLDPC(indexNeg)=gammaupdated(indexNeg)./GAMMAupdated(indexNeg);
    varLDPC(indexNeg)=1./GAMMAupdated(indexNeg);  
end

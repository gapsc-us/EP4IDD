...
%dataEP.numberSimulations=100; %number of diferent simulations, or average using MC
%dataEP.numberFrames=100;  %number of words transmitted per simulation
...
% EbN0 range
dataEP.EbNoIni=9.5-3;
dataEP.EbNoStep=.25;
dataEP.EbNoEnd=11-3;
%Different EbN0 can be selected for some methods
scenario.EbNoEP=[dataEP.EbNoIni,dataEP.EbNoEnd];
scenario.EbNoMMSE=[dataEP.EbNoIni,dataEP.EbNoEnd]; 
scenario.EbNoOptimal=[dataEP.EbNoIni,dataEP.EbNoEnd]; 
...
dataEP.numberAntennas=[8 64]; %not used if equalization, but should not be empty 
...
% Particular Range of EbNo for algorithms

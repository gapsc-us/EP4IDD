...
% dataEP.dataEP.M=64; %Modulation order
% dataEP.complexFlag=complexFlag;
% 
% dataEP.dataEP.numberTaps=dataEP.numberTaps;
% dataEP.dataEP.numberAntennas=dataEP.numberAntennas;
% dataEP.dataEP.numberChannels=dataEP.numberChannels;
% dataEP.channel_h=channel_h;
% dataEP.SNRdBIni=4;
% dataEP.SNRdBStep=2;
% dataEP.SNRdBEnd=20;
% dataEP.dataEP.flagPSK=dataEP.flagPSK;
% dataEP.dataEP.numberTurbo=5; % number of turbo iterations. If no turbo (standalone
%                       %equalization), set it to 0.
% dataEP.varnoiseCH=varnoiseCH;
% dataEP.flagInterleaving=flagInterleaving;
% dataEP.dataEP.flagIterDec=dataEP.flagIterDec;
% dataEP.folderSave='Results';
...
if dataEP.scenario==1 % equalization
    ...
    name=([dataEP.folderSave,'/','resultsEPeq', '_b' num2str(dataEP.bits) '_M' num2str(dataEP.M) '_NtxNr' num2str(dataEP.numberAntennas(1)) 'x' num2str(dataEP.numberAntennas(2)) '_n' num2str(n) '_numCh' num2str(dataEP.numberChannels) '_' ]);
end
...
if dataEP.numberTurbo==0
    % Hard
    flagBEP=(sum(BER_hard_BEP));
    flagMMSE=(sum(BER_hard_MMSE));
    flagSEP=(sum(BER_hard_SEP));
    flagOptimal=(sum(BER_hard_Optimal));
    flagEPICLMMSE=sum(BER_hard_EPICLMMSE);
    flagPKSEP=sum(sum(BER_soft_PKSEP_turbo));
    flagPBEP=sum(BER_hard_PBEP);
    flagDBEP=sum(BER_hard_DBEP);
    flagPFEP=sum(BER_hard_PFEP);
    flagFMMSE=sum(BER_hard_FMMSE);

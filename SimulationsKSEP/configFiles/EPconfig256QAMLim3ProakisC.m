...
dataEP.scenario=1; % 1 for 'Equalization' or 2 for 'MIMO'

% Monte Carlo is used, with numberSimulations experiments with a
% transmission of numberFrames coded words
dataEP.numberSimulations=500; %number of diferent simulations, or average using MC
dataEP.numberFrames=20;  %number of words transmitted per simulation
...
% EbN0 range
dataEP.SNRdBIni=26-3;
dataEP.SNRdBStep=1;
dataEP.SNRdBEnd=42-3;
...
%EP inner iterations
 ...
 dataEP.numberTurbo=10; % number of turbo iterations. If no turbo (standalone
                      %equalization), set it to 0.

% LDCP channel coded block length
    dataEP.channelBlockLength=4096;
    dataEP.bits=log2(dataEP.channelBlockLength)-1; %rate 1/2
    dataEP.flagBlockLengths=0; %to 1 if several values are used to compare
    %This value must be set in dataEP.channelBlockLength when calling mainEP
    ...
    dataEP.flagInterleaving=1; % To 1 if interleaving and deinterleaving is included.
...
% Channel paramenters
dataEP.channel_h=[ 1 2 3 2 1]/sqrt(19); %[ 1 2 3 2 1]/sqrt(19); % channel_h=[] means random channels. We can also set a  
dataEP.channelName='ProakisC'; %Just to save results
dataEP.numberAntennas=[0 0]; %not used if equalization, but should not be empty 
...
dataEP.numberChannels=1;  %number of random channels. If channel_h is not empty
...
switch dataEP.scenario
    case 1,
        ...
        scenario.flagSEP=1; % SEP [Santos17]
        scenario.flagPKSEP=1; % P-KSEP [Santos18c]
    ...
    case 2
        ...
        scenario.flagMMSEAproxGS=0; % approximated block LMMSE with GS
        scenario.flagMMSEAproxNeuman=0; % approximated block LMMSE with Neuman
        scenario.flagMMSEAproxPCG=0; % approximated block LMMSE with PCG
        scenario.flagDBEPAproxGS=0; % approximated D-BEP with GS
        scenario.flagDBEPAproxNeuman=0; % approximated D-BEP with Neuman

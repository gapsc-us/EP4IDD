...
dataEP.numberSimulations=2; %number of diferent simulations, or average using MC
dataEP.numberFrames=5;  %number of words transmitted per simulation
...
% EbN0 range
dataEP.SNRdBIni=42-3;
dataEP.SNRdBStep=3;
dataEP.SNRdBEnd=50-3;
...
% Channel paramenters
csi=[linspace(0,1,20),linspace(1,0,40)];
csi=csi/norm(csi);
dataEP.channel_h=csi;
%dataEP.channel_h=[]; %[ 1 2 3 2 1]/sqrt(19); % channel_h=[] means random channels. We can also set a  
dataEP.channelName='Proakis60'; %Just to save results
